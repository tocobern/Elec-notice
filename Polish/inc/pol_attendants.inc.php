...
<?php
	...
	if(isset($_POST) && array_key_exists('meetingCount',$_POST) && is_numeric($_POST['meetingCount'])) { 
		...
		if ($_POST['clearAttendance']=="checked"){
			file_put_contents("..//pol-meeting-attendance.txt","");//overwrite file.
		}else{
		...
		//WRITE TO TEXT FILE.
		file_put_contents("..//pol-meeting-attendance.txt", $meetingCount, FILE_APPEND | LOCK_EX);
		...
		$sql = 	"INSERT INTO `meeting_attendance`(`ID`, `Date`, `Congregation`, `Count`) 
		VALUES ('id','$dateForDB','Polish','$meetingCount')";
