...
<?php
	...
	if(isset($_POST) && array_key_exists('meetingCount',$_POST) && is_numeric($_POST['meetingCount'])) { 
		...
		if ($_POST['clearAttendance']=="checked"){
			file_put_contents("..//lv-meeting-attendance.txt","");//overwrite file.
		}else{
			echo "clear attendants not set";
		}
 
		//WRITE TO TEXT FILE.
		file_put_contents("..//lv-meeting-attendance.txt", $meetingCount, FILE_APPEND | LOCK_EX);
		...
		VALUES ('id','$dateForDB','Jubilee Park','$meetingCount')";*/
				

		$sql = 	"INSERT INTO `meeting_attendance`(`ID`, `Date`, `Congregation`, `Count`) 
		VALUES ('id','$dateForDB','Lea valley','$meetingCount')";
